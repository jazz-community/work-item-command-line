...
It is possible to work with the **WCL source code**. It is possible to import the code of WCL into Eclipse and build your own WCL. The WCL requires the Plain Java Client Libraries to be accessible in the Eclipse project. See the [RTC Extensions Workshop](https://jazz.net/library/article/1000) especially Lab 1 that helps setting up the EWM SDK and the Plain Java Client Libraries. Please see [the blog rsjazz.wordpress.com](https://rsjazz.wordpress.com/) for updates on how to perform the workshop with newer versions of EWM. Follow the Lab 1 to setup the SDK and development environmnet. Import WCL into this environment either the server development workspace or the client development workspace. Both should work.
